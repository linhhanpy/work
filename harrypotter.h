...
int street()
{
	...
	case 0:
		if (camp) {
			int f;
			cout << "你遇到了科林·克里维" << endl;
			cout << "1进攻/2逃跑" << endl;
			cin >> f;
			if (f == 1) {
				fight("克里维", 500, 110, 0, 14, 1000);
				if (!dead && rand() % 2) {
					if (rand() % 4 == 0) {
						cout << "获得一瓶经验药水" << endl;
						bag[22] += 1;
			...
			int f;
			cout << "你遇到了文森特·克拉布" << endl;
			cout << "1进攻/2逃跑" << endl;
			cin >> f;
			if (f == 1) {
				fight("克拉布", 500, 110, 0, 15, 1000);
				if (!dead && rand() % 2) {
					if (rand() % 4 == 0) {
						cout << "获得一瓶经验药水" << endl;
						bag[22] += 1;
	...
	case 1:
		if (camp) {
			int f;
			cout << "你遇到了赫敏·格兰杰" << endl;
			cout << "1进攻/2逃跑" << endl;
			cin >> f;
			if (f == 1) {
				fight("格兰杰", 2000, 500, 15, 15, 1000);
				if (!dead && rand() % 3 != 0) {
					if (rand() % 5 == 0) {
						cout << "获得两瓶经验药水" << endl;
						bag[22] += 2;
			...
			int f;
			cout << "你遇到了德拉科·马尔福" << endl;
			cout << "1进攻/2逃跑" << endl;
			cin >> f;
			if (f == 1) {
				fight("马尔福", 2000, 500, 15, 15, 1000);
				if (!dead && rand() % 3 != 0) {
					if (rand() % 5 == 0) {
						cout << "获得两瓶经验药水" << endl;
						bag[22] += 2;
	...
	case 2:
		if (camp) {
			int f;
			cout << "你遇到了小天狼星布莱克" << endl;
			cout << "1进攻/2逃跑" << endl;
			cin >> f;
			if (f == 1) {
				fight("布莱克", 5000, 700, 50, 16, 1500);
				if (!dead && rand() % 4 != 0) {
					if (rand() % 5 == 0) {
						cout << "获得三瓶经验药水" << endl;
						bag[22] += 3;
			...
			int f;
			cout << "你遇到了奥古斯特·卢克伍德" << endl;
			cout << "1进攻/2逃跑" << endl;
			cin >> f;
			if (f == 1) {
				fight("卢克伍德", 5000, 700, 50, 15, 1500);
				if (!dead && rand() % 4 != 0) {
					if (rand() % 5 == 0) {
						cout << "获得三瓶经验药水" << endl;
						bag[22] += 3;
	...
	case 5:
		if (camp) {
			...
			cin >> f;
			if (f == 1) {
				fight("纳吉尼", 300000, 36000, 7500, 20, 5000);
				if (!dead) {
					if (rand() % 3) {
						if (rand() % 2) {
							cout << "获得技能书恢复如初" << endl;
...
int LD()
{
	...
	if (a == 1) {
		...
		if (bag[0] >= num) {
			...
			cout << "经验石X          " << num5+10000000 << "个" << endl;
			cout << "生命药水         " << num6+10000 << "个" << endl;
			cout << "防御药水         " << num7 << "个" << endl;
			cout << "攻击药水         " << num8 << "个" << endl;
			cout << "经验药水         " << num9 << "个" << endl;
			cout << "全幅药水         " << num10+100000 << "个" << endl;
			cout << "金币             " << num11+10000 << "个" << endl;
		}
...
int clearance()
{
	...
	if (camp) {
		...
		cout << "哈利·波特, 你看到这张纸条的时候, 你们的校已经掌握在我手中, 要么你前来送死，要么我就会把你的同伙们统统杀死！！！"<<endl;
		cout << "                                                                                               ——伏地魔" << endl;
	}
	_getch();
	f = 1;
...
int MM()
{
	if (amm == 0) {
		...
		if (a == 1) {
			...
			Sleep(1000);
			cout << "你遇到了副部长多洛雷斯·乌姆里奇" << endl;
			Sleep(1000);
			fight("乌姆里奇", 10000, 1000, 100, 20, 3000);
			health += 5000;
			if (dead) return 0;
			if (lie) {
				cout << "上次的员工跑了过来" << endl;
